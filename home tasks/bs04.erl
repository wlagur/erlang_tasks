...
decode_to_proplist(<<"}">>, [{Key,Value}|Acc]) ->
	p05:reverse([{check(Key),check(Value)}|Acc]);
...
decode_to_proplist(<<"[",RestText/binary>>, [{Key,_Value}|Acc]) ->
	...
	decode_to_proplist(Outside_json, [{Key, p05:reverse([ decode_to_proplist(X, [{<<>>,<<>>}]) || X<-Inside_list ])}|Acc]);
...
decode_to_map(<<"[",RestText/binary>>, Acc, Key, _Value) ->
	{Inside_list, Outside_json} = splite_inside_list(1, RestText, <<>>, []),
	decode_to_map(Outside_json, Acc, Key, p05:reverse([ decode_to_map(X, #{}, <<>>, <<>>) || X<-Inside_list ]));
...
check(<<X/utf8,RestText/binary>>) ->
	case X of
		$' -> binary:part(RestText, 0, byte_size(RestText) - 1);
